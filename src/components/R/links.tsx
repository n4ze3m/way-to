export const RLinks = ({ path, links }: { path: string; links: any }) => {
  return (
        ...
        <div className="grid grid-cols-1 gap-4">
          {links.map((link: any) => (
            <a href={link.url} className="-space-y-px rounded-md shadow-sm">
              <div className="flex flex-col items-start justify-start space-y-3 rounded-md border border-gray-300 bg-white px-6 py-3">
                <span
                ...
                <span title={link.url} className="text-xs text-gray-500">
                  {link.url}
                </span>
              </div>
            </a>
